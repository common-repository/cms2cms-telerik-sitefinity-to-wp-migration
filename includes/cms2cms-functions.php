...
/**
 * Class CmsPluginFunctionsSitefinity
 */
class CmsPluginFunctionsSitefinity
{
    ...
    private $jsConfig = '{
        ...
        "wp_feedback"  : "//wordpress.org/support/plugin/cms2cms-telerik-sitefinity-to-wp-migration/reviews",
    ...
    {
        $cms2cms_sitefinity_access_login = get_option('cms2cms-sitefinity-login');
        $cms2cms_sitefinity_access_key   = get_option('cms2cms-sitefinity-key');

        return array(
            'email'     => $cms2cms_sitefinity_access_login,
            'accessKey' => $cms2cms_sitefinity_access_key
        );
    }
    ...
    public function isActivated()
    {
        ...
        return ($cms2cms_sitefinity_access_key != false);
    }
    ...
    public function getOptions()
    {
        ...
        $login = get_option('cms2cms-sitefinity-login');
    ...
    public function saveOptions()
    {
        ...
        if ( $key && $login ) {
            ...
            add_option('cms2cms-sitefinity-key', $key);

            delete_option('cms2cms-sitefinity-login');
            add_option('cms2cms-sitefinity-login', $login);

            delete_option('cms2cms-sitefinity-depth');
            add_option('cms2cms-sitefinity-depth', $bridge_depth);
    ...
    {
        return $this->__('Sitefinity', 'cms2cms-sitefinity-migration');
    }
    ...
    {
        return $this->__('WordPress', 'cms2cms-sitefinity-migration');
    }
    ...
    {
        return sprintf(
            $this->__('CMS2CMS: Automated %s to %s Migration ', 'cms2cms-sitefinity-migration'),
    ...
    {
        return sprintf(
            $this->__('%s to %s', 'cms2cms-sitefinity-migration'),
            $this->getPluginSourceName(),
            $this->getPluginTargetName()
        );
    }
    ...
    public function logOut()
    {
        if (isset($_REQUEST['_wpnonce'])) {
            ...
            if ($this->verifyFormTempKey($nonce, 'cms2cms_sitefinity_logout')
                && $_POST['cms2cms_sitefinity_logout'] == 1
            ) {
                $this->clearOptions();
            }
        }
